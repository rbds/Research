...
x2_str = 0 #absolute distance of the obstacle (x)
y2_str = 0 #absolute distance of the obstacle (y)
...
while len(all_index) < 50: 
    ...
    if (index==1 or index==0): #Plot the robot's path
        ...
        dist_traveled = dist_traveled + dst
...
#    if thermoVoltage>=1: #Plot sources of geothermal power
#        plt.figure(1)
#        plt.subplot(211)
...
#    old_thermo = thermoVoltage
    ...
    plt.figure(1)
    plt.draw()
    dist_traveled_str = str(dist_traveled)
    x2_str = str(x2)
    y2_str = str(y2)
    msg = msg + ' ' + dist_traveled_str + ' ' + x2_str + ' ' + y2_str
