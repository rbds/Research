...
Probability of traverse is a pre-calculated metric related to chance constraints \cite{Blackmore - Robust path planning}, although it is not an exact parallel. 
...
Among many examples, work by Bry and Roy \cite{belief space} considers motion uncertainty, or uncertainty in the state estimate, during path planning. 
Sensing uncertainty, generated by the uncertainty of measurements, is addressed by work such as \cite{(Luders et al. - Bounds on Tracking Error Using Closed Loop RRTs)} or \cite{(LQG-MP)}. 
Environmental uncertainty is defined as the uncertainty in the placement of obstacles and is addressed by \cite{(Roughness based RRT,} \cite{ particle filter RRT)} in the context of probabilistic navigation algorithms. 
...
%\begin{align}
% \displaystyle{\min_{x\in W} J(x, SOC) &=\sum_{i=1}^{k} H_i \\
...
figure of costs and probabilities of traverse.
%\begin{figure}[h!]
%\centering
%\includegraphics[scale=0.5]{RCP_soln}
...
figure of best path through environment.
