...
% for i=num:-1:(num-V+1) %create d vector to store cost, P_tr, parents for each entry in adj.
%    d{i,1} = [0 1 num];
% end

for i=col_last:-1:1 %counting back from last populated row in adjacency matrix,
    col = mod(i, V)+1;
    conns = find(adj(:,i)~=0)
